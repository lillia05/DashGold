...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $whereClause .= " AND (nama_barang LIKE '%$search%' OR supplier LIKE '%$search%')";
}
...
$query = mysqli_query($conn, "SELECT * FROM stocks $whereClause ORDER BY created_at DESC");
?>
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8 relative">
                    ...
                    <table class="w-full text-left whitespace-nowrap">
                        <thead class="bg-white text-gray-500 uppercase text-xs border-b border-gray-100">
                            <tr>
                                ...
                                <th class="px-6 py-3 font-semibold bg-gray-50/50">Tanggal Beli</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            ...
                            <tr class="table-row">
                                ...
                                <td class="px-6 py-4 text-gray-900 font-medium"><?= date('d M Y', strtotime($row['tanggal_beli'])); ?></td>
                                <td class="px-6 py-4 text-right text-gray-500"><?= number_format($row['harga_beli_total'], 0, ',', '.'); ?></td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-3">
                                        <button onclick="bukaModalDetail('<?= $row['nama_barang'] ?>', '<?= $row['supplier'] ?>', '<?= $row['berat'] ?>', '<?= $row['status'] ?>', '<?= date('d M Y', strtotime($row['tanggal_beli'])) ?>', '<?= number_format($row['harga_beli_total'],0,',','.') ?>')" class="text-emerald-500 hover:text-emerald-700 transition" title="Lihat Detail">
            ...
            <div id="modalEdit" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center z-50 backdrop-blur-sm p-4">
                <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden overflow-y-auto max-h-[90vh]">
                    ...
                    <form class="p-6 md:p-8" action="proses_stok.php?act=update" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input name="harga_beli" id="editHarga" type="number" class="w-full border border-gray-300 rounded-lg pl-10 p-2.5 text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
                                </div>
                            </div>
                            ...
                            <div class="col-span-1 md:col-span-2 pt-4 border-t border-dashed border-gray-200">
                                ...
                                <p class="text-xs text-gray-500 mt-1 italic">*Ubah ke "Terjual" untuk mencatat penjualan & hitung profit.</p>
                            </div>
                            ...
                            <div id="salesFields" class="hidden col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 bg-emerald-50 p-4 rounded-lg border border-emerald-100">
                                ...
                                <div>
                                    ...
                                    <input name="tanggal_jual" id="editTglJual" type="date" value="<?= date('Y-m-d'); ?>" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Harga Jual (Total)</label>
                                    <div class="relative">
                                        <span class="absolute left-3 top-2.5 text-gray-500 text-sm font-medium">Rp</span>
                                        <input name="harga_jual" id="editHargaJual" type="number" class="w-full border border-gray-300 rounded-lg pl-10 p-2.5 text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl transform transition-all scale-100 max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <div id="detailNama" class="font-bold text-gray-900 text-lg"></div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Supplier</label>
                            <div id="detailSupplier" class="font-bold text-gray-800"></div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Stok (Gr)</label>
                            <div id="detailStok" class="font-bold text-gray-800"></div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Beli</label>
                            <div id="detailTgl" class="font-bold text-gray-800"></div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Harga Beli</label>
                            <div id="detailHarga" class="font-bold text-gray-800"></div>
                        </div>
                    </div>
    ...
    <script>
        ...
        document.getElementById('editStatus').addEventListener('change', function() {
            ...
            if (this.value === 'sold') {
                ...
                document.getElementById('editHargaJual').setAttribute('required', 'required');
            } else {
                salesFields.classList.add('hidden');
                document.getElementById('editPembeli').removeAttribute('required');
                document.getElementById('editHargaJual').removeAttribute('required');
            }
        });

        function bukaModalEdit(id, nama, supplier, stok, status, tglBeli, hargaBeli) {
            ...
            document.getElementById('editHarga').value = hargaBeli;
        ...
        function bukaModalDetail(nama, supplier, stok, status, tgl, harga) {
            document.getElementById('detailNama').innerText = nama;
            document.getElementById('detailSupplier').innerText = supplier;
            document.getElementById('detailStok').innerText = stok + ' Gr';
